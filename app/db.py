...
try:
    from .config import get_db_settings, get_shard_settings
    from .sharded_db import get_sharded_db
except ImportError:
    from config import get_db_settings, get_shard_settings
    from sharded_db import get_sharded_db
...
def get_sharded_db_layer():
    """Get ShardedDBLayer instance for accessing sharded database."""
    settings = get_shard_settings()
    return get_sharded_db(settings["user"], settings["password"], settings["database"])
